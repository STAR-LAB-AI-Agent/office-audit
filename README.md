...
审计结果包含目标文件、审计范围、统计摘要、问题列表、未审计对象列表和运行指标。每条问题记录唯一编号、严重级别、规则编号、结构化位置、证据、问题说明和修复建议；Markdown 报告另给出“正文第 N 段”、当前段落或相邻可见文字，格式离群还列出具体属性差异。每个未审计对象也单独记录类型、位置、数量、未审计原因、影响和建议，不能只输出一句笼统的“存在未审计对象”。
